...
const client = new SyncClient({
  ...
  clientType: 'web',
  onPaired: (token) => {
    console.log('Share this token:', token); // e.g., "A3F9K2"
  },
  onSync: (payload) => {
...
const client = new SyncClient({
  serverUrl: 'ws://localhost:8080',
  clientType: 'figma',
  sessionToken: 'A3F9K2', // Token from web client
  onSync: (payload) => {
...
```json
{
  ...
  "sessionToken": "A3F9K2" // Only for Figma
}
```
...
- **Token-based pairing**: Simple 6-character tokens (e.g., `A3F9K2`)
...
```
Session Lifecycle:
1. Web connects → Server generates token (e.g., "A3F9K2")
2. User copies token
